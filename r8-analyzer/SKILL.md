...
book_path: /build/_book.yaml
project_path: /build/_project.yaml
...
keywords_public: R8, proguard, keep rules, app size, optimization
...
- \[ \] Step 3: If the AGP version is less than 9, suggest moving to AGP 9.0 version as AGP 9.0 includes optimizations(https://developer.android.com/topic/performance/app-optimization/enable-app-optimization#agp-r8-behavior-changes).
...
- \[ \] Step 9: Advise the user to run tests using [UI
  automator](references/android/training/testing/other-components/ui-automator.md) to assess that there is no issue with the suggested changes, concentrating on the packages where keep rules will be affected.
